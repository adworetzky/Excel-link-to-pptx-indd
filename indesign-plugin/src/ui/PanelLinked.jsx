/**
 * PanelLinked — shown when a linked text frame is selected.
 * Displays source info, last value, Update Now, Remove Link, Auto-update toggle.
 */
...
export default function PanelLinked({ link, onUpdateNow, onRemoveLink, onToggleAutoUpdate, onRepointFile }) {
  ...
  async function handleRemove() {
    if (!window.confirm("Remove the DataLink from this frame? The text content will not change.")) return;
    setRemoving(true);
    try {
      await onRemoveLink();
    } finally {
      ...
      {isFileBroken && (
        <div style={styles.warningBox}>
          <p style={styles.warningText}>
            The Excel file could not be found at the stored path. The link is broken.
          </p>
      ...
      <div style={styles.toggleRow}>
        <label style={styles.toggleLabel}>
          <input
            ...
            style={{ marginRight: 6 }}
          />
          Auto-update when InDesign activates
        </label>
      </div>
