/**
 * PanelNotLinked — shown when a shape is selected but has no DataLink.
 * File browse uses an HTML file input (Office JS taskpane is a browser).
 */
import React, { useState, useRef } from "react";
import { getSheetNames } from "@datalink/shared";
...
export default function PanelNotLinked({ onLink }) {
  ...
  async function handleFileChange(e) {
    ...
    // In Office JS desktop taskpane, File.path is available on Windows
    const path = file.path || file.name;
    setFilePath(path);
    setErrors((prev) => ({ ...prev, file: undefined }));
    setSheetLoading(true);
    try {
      const names = await getSheetNames(path);
      setSheets(names);
      setSelectedSheet(names[0] || "");
    } catch (err) {
      setErrors((prev) => ({ ...prev, file: `Cannot read file: ${err.message}` }));
      setSheets([]);
      setSelectedSheet("");
    } finally {
